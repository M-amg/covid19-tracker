...
public class CovidCasesController extends ResponseEntityExceptionHandler {
    ...
    @ResponseStatus(HttpStatus.CREATED)
    public CovidCases update(@RequestParam("id") @Min(1) Long id, @Valid @RequestBody CovidCases cases) throws Exception {
        return covidCasesService.updateCase(id,cases);
    }

    @GetMapping(path = "/covid-cases")
    public ResponseEntity<Map<String, Object>> getCase(@Param("date") @NotBlank(message = "Please provide a date") String date,
                                                       @Param("type") @NotNull(message = "Please provide a PreDateType") PreDateType type,
                                                       @Param("countries") @NotNull(message = "Please provide a Countries")  Countries[] countries,
                                                       @Param("test_result")  @NotNull(message = "Please provide a TestResult") TestResult testResult,
